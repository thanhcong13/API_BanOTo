...
namespace DoAn.OTo.Infrastrure.Repository
{
    ...
    {
        public async Task<IEnumerable<CTLienHe>> GetPage(int page, int pageSize)
        {
            using (SqlConnection = new MySqlConnection(ConnectionString))
            {
                var offset = (page - 1) * pageSize;
                var query = $"SELECT * FROM CTLienHE LIMIT @PageSize OFFSET @Offset";
                return await SqlConnection.QueryAsync<CTLienHe>(query, new { PageSize = pageSize, Offset = offset });
            }
        }
    }
}
